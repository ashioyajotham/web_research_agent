"""Tests for the _read_query multiline input helper in webresearch/cli.py."""
import sys
import builtins
from unittest.mock import patch, MagicMock
import pytest

# webresearch/cli.py has display-only deps (pyfiglet, questionary) that may
# not be installed in the test environment — stub them before importing.
...
def _import_read_query():
    ...
    from webresearch.cli import _read_query
    return _read_query
...
def test_single_line_query_returned_immediately(monkeypatch):
    ...
    monkeypatch.setattr("webresearch.cli.Prompt.ask", _make_prompt_ask("What is the capital of France?"))
    from webresearch.cli import _read_query
...
def test_back_returns_immediately(monkeypatch):
    monkeypatch.setattr("webresearch.cli.Prompt.ask", _make_prompt_ask("back"))
    from webresearch.cli import _read_query
    result = _read_query("Research question (or 'back')")
    assert result == "back"
...
def test_empty_input_returns_empty(monkeypatch):
    monkeypatch.setattr("webresearch.cli.Prompt.ask", _make_prompt_ask(""))
    from webresearch.cli import _read_query
    result = _read_query("Research question (or 'back')")
    assert result == ""
...
def test_colon_triggers_multiline_continuation(monkeypatch):
    ...
    monkeypatch.setattr("webresearch.cli.Prompt.ask", _make_prompt_ask("Compile companies matching:"))
    stdin_lines = iter(["  - Based in the EU", "  - Revenue > 1B EUR", ""])
    monkeypatch.setattr("builtins.input", lambda: next(stdin_lines))
    monkeypatch.setattr("webresearch.cli.console", MagicMock())
...
def test_multiline_eof_handled_gracefully(monkeypatch):
    ...
    monkeypatch.setattr("webresearch.cli.Prompt.ask", _make_prompt_ask("Criteria:"))
...
def test_multiline_keyboard_interrupt_handled_gracefully(monkeypatch):
    ...
    monkeypatch.setattr("webresearch.cli.Prompt.ask", _make_prompt_ask("List:"))
